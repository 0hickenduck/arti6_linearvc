...
describe("regression: Gemini CLI 0.40.x template compatibility (#224)", () => {
  ...
  it("[#224] gemini agent descriptions make the routing intent legible", () => {
    const agentsDir = path.resolve(
      repoRoot,
      "packages/cli/src/templates/gemini/agents",
    );
    const research = fs.readFileSync(
      path.join(agentsDir, "trellis-research.md"),
      "utf-8",
    );
    const implement = fs.readFileSync(
      path.join(agentsDir, "trellis-implement.md"),
      "utf-8",
    );
    const check = fs.readFileSync(
      path.join(agentsDir, "trellis-check.md"),
      "utf-8",
    );

    expect(research).toContain("Gemini Flash research specialist");
    expect(research).toContain("research/");
    expect(implement).toContain("Gemini Pro implementation specialist");
    expect(implement).toContain("task context");
    expect(check).toContain("Gemini Pro verification specialist");
    expect(check).toContain("lint plus type-check");
  });

  it("[#224] gemini settings.json pins trellis agents to Flash/Pro models", () => {
    const settingsPath = path.resolve(
      repoRoot,
      "packages/cli/src/templates/gemini/settings.json",
    );
    ...
    const parsed = JSON.parse(raw) as {
      agents?: {
        overrides?: Record<string, { modelConfig?: { model?: string } }>;
      };
    };

    expect(
      parsed.agents?.overrides?.["trellis-research"]?.modelConfig?.model,
    ).toBe("gemini-3-flash-preview");
    expect(
      parsed.agents?.overrides?.["trellis-implement"]?.modelConfig?.model,
    ).toBe("gemini-3.1-pro-preview");
    expect(
      parsed.agents?.overrides?.["trellis-check"]?.modelConfig?.model,
    ).toBe("gemini-3.1-pro-preview");
  });
