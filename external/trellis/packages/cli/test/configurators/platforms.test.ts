...
describe("configurePlatform", () => {
  ...
  it("configurePlatform('gemini') pins trellis roles to Flash/Pro models", async () => {
    await configurePlatform("gemini", tmpDir);
    ...
    expect(
      settings.agents?.overrides?.["trellis-research"]?.modelConfig?.model,
    ).toBe("gemini-3-flash-preview");
    expect(
      settings.agents?.overrides?.["trellis-implement"]?.modelConfig?.model,
    ).toBe("gemini-3.1-pro-preview");
    expect(
      settings.agents?.overrides?.["trellis-check"]?.modelConfig?.model,
    ).toBe("gemini-3.1-pro-preview");
  });
